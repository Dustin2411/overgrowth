...
PYBIND11_MODULE(og_rl_interface, m) {
    ...
    // Debug logging
    std::cout << "[DEBUG] Initializing og_rl_interface module bindings" << std::endl;

    // Enable thread safety annotations for PyBind11
    py::class_<OvergrowthEnv, std::shared_ptr<OvergrowthEnv>>(m, "OvergrowthEnv")
        .def(py::init([]() {
            std::cout << "[DEBUG] Creating OvergrowthEnv instance via lambda init" << std::endl;
    ...
    std::cout << "[DEBUG] Bound OvergrowthEnv class in og_rl_interface" << std::endl;

    // Enable GIL management for thread safety
    m.attr("PYBIND11_THREAD_SAFETY_ANNOTATION") = py::cast(true);

    std::cout << "[DEBUG] og_rl_interface module initialization complete" << std::endl;
}
PYBIND11_MODULE(og_rl_interface, m) {
    m.doc() = "Overgrowth Reinforcement Learning Environment Interface";
