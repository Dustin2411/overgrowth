...
// RL Agent Control Variables
bool rl_control_active = false;  // When true, RL overrides normal AI
vec3 rl_target_velocity = vec3(0, 0, 0);  // Movement velocity from RL
bool rl_wants_to_attack = false;
bool rl_wants_to_jump = false;
bool rl_wants_to_crouch = false;
bool rl_wants_to_block = false;
bool rl_wants_to_throw = false;
...
void MindReceiveMessage(string msg) {
    ...
    // RL Agent Action Handlers - These override normal AI when RL is controlling
    if(token == "rl_forward") {
        rl_control_active = true;
        rl_target_velocity = this_mo.GetFacing() * run_speed;
        rl_wants_to_attack = false;
        rl_wants_to_jump = false;
        Log(info, "[RL] NPC " + this_mo.getID() + " moving forward");
        return;
    } else if(token == "rl_back") {
        rl_control_active = true;
        rl_target_velocity = this_mo.GetFacing() * -run_speed * 0.5;
        rl_wants_to_attack = false;
        Log(info, "[RL] NPC " + this_mo.getID() + " moving back");
        return;
    } else if(token == "rl_left") {
        rl_control_active = true;
        vec3 facing = this_mo.GetFacing();
        rl_target_velocity = vec3(facing.z, 0, -facing.x) * run_speed * 0.5;
        rl_wants_to_attack = false;
        Log(info, "[RL] NPC " + this_mo.getID() + " moving left");
        return;
        ...
        rl_target_velocity = vec3(-facing.z, 0, facing.x) * run_speed * 0.5;
        rl_wants_to_attack = false;
        Log(info, "[RL] NPC " + this_mo.getID() + " moving right");
        return;
    } else if(token == "rl_jump") {
        rl_control_active = true;
        rl_wants_to_jump = true;
        rl_target_velocity = vec3(0, 0, 0);
        Log(info, "[RL] NPC " + this_mo.getID() + " jumping");
        return;
    } else if(token == "rl_crouch") {
        rl_control_active = true;
        rl_wants_to_crouch = true;
        rl_target_velocity = vec3(0, 0, 0);
        Log(info, "[RL] NPC " + this_mo.getID() + " crouching");
        return;
    } else if(token == "rl_attack") {
        rl_control_active = true;
        rl_wants_to_attack = true;
        ai_attacking = true;  // Also set the original flag for compatibility
        Log(info, "[RL] NPC " + this_mo.getID() + " attacking");
        return;
    } else if(token == "rl_block") {
        rl_control_active = true;
        rl_wants_to_block = true;
        going_to_block = true;  // Set the original blocking flag
        Log(info, "[RL] NPC " + this_mo.getID() + " blocking");
        return;
    } else if(token == "rl_throw") {
        rl_control_active = true;
        rl_wants_to_throw = true;
        throw_after_active_block = true;  // Trigger throw behavior
        Log(info, "[RL] NPC " + this_mo.getID() + " throwing");
        return;
    } else if(token == "rl_grab") {
        rl_control_active = true;
        rl_wants_to_throw = true;  // Grab leads to throw in this game
        Log(info, "[RL] NPC " + this_mo.getID() + " grabbing");
        return;
    } else if(token == "rl_idle") {
        rl_control_active = true;
        rl_target_velocity = vec3(0, 0, 0);
        rl_wants_to_attack = false;
        rl_wants_to_jump = false;
        rl_wants_to_crouch = false;
        rl_wants_to_block = false;
        rl_wants_to_throw = false;
        return;
    }
...
bool WantsToCrouch() {
    // RL override
    if(rl_control_active && rl_wants_to_crouch) {
        rl_wants_to_crouch = false;  // Reset after use
        return true;
    }
...
bool WantsToJump() {
    // RL override
    if(rl_control_active && rl_wants_to_jump) {
        rl_wants_to_jump = false;  // Reset after use
        return true;
    }
...
bool WantsToAttack() {
    // RL override
    if(rl_control_active && rl_wants_to_attack) {
        rl_wants_to_attack = false;  // Reset after use
        return true;
    }
...
vec3 GetTargetVelocity() {
    // RL override - return RL-controlled velocity if active
    if(rl_control_active) {
        return rl_target_velocity;
    }
